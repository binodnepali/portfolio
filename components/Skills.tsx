...
export default function SkillsSection({ skills }: { skills: Skill[] }) {
  ...
  // Skills with include_in_cv === false stay on the site but are excluded from
  // the printed CV. If none are included, hide the whole section in print.
  const cvSkillCount = skills.filter((s) => s.include_in_cv !== false).length;

  return (
    <Section title="Skills" class={cvSkillCount === 0 ? "print:hidden" : ""}>
